...
-- 현재 설치된 Oracle 에 포함된 모든 Table 를 보여달라
SELECT * FROM all_tables;

-- iolist User 가 접근(CRUD 실행) 할수 있는 table 를 보여달라
...
INSERT INTO tbl_buyer ( buid, buname, butel, buaddr)
VALUES('0000000003', '이몽룡', '010-3333-3333', '한양 특별시');

INSERT INTO tbl_buyer ( buid, buname, butel, buaddr)
VALUES('0000000004', '김춘향', '010-4444-4444', '함경도 함흥');

INSERT INTO tbl_buyer ( buid, buname, butel, buaddr)
VALUES('0000000005', '이길룡', '010-5555-5555', '광주광역시');

COMMIT;

SELECT * FROM tbl_buyer;

-- PK 를 기분으로 조회(SELECT) 하면 결과는 1개 이거나 null 이다
SELECT * FROM tbl_buyer WHERE buid = '0000000001';
SELECT * FROM tbl_buyer WHERE buid = '0000000007';

-- PK 이외의 칼럼을 기준으로 조회 하면 비록 1개의 결과만 보일지라도
-- 이 결과는 1개 이상인 것으로 본다.
...
/*
중간문자열 검색
Full Text Search : 칼럼의 데이터의 무자열을 모두 검색하여 일치하는 데이터 찾기
...
허용하지 않는다.
그래서 의미없는 DUMMT TABLE 을 하나 마련해 두었다.
Oracle 에서 단순 연산을 위한 SELECT 에서는 dual Table 를 사용하여 수행한다.
*/
