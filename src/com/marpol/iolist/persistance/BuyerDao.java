...
import com.marpol.iolist.persistance.sql.BuyerSQL;

/*
 * DBMS에 Query 를 보내고, 데이터를 받을 method 선언
 * 인터페이스에 Query method 를 선언만 해두면
 * Mybatis 의 SessionFactory
 * 		가 실제 사용될 코드를 자동으로 작성한다
 */
public interface BuyerDao {
	...
	/*
	 * 고객정보를 관리하기 위해서 추가할 기능 1. 고객 이름으로 조회하기 2. 고객 전화번호로 조회하기
	 * 
	 */
	...
	/*
	 * select(조회) method 를 만들때 주의 사항 조회대상(where 절에서 사용)이 pk 일때는 조회되는 데이터가 없거나(null),
	 * 1개 뿐이다 이때 method 의 return type 은 Dto 로 설정 public BuyerDto findById(String
	 * buId); SELECT * FROM tbl_buyer WHERE buid = '000001'
	 * 
	 * 조회 대상이 PK 가 아닌경우 조회되는 데이터는 없거나(empty), 1개 이상이다 이때 method 의 return type 은
	 * List<Dto> 로 설정 public List<BuyerDto> findByName(String name); public
	 * List<BuyerDto> findByTel(String tel);
	 */
