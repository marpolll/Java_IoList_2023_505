...
public class BuyerServiceImplV2 extends BuyerServiceImplV1 {
	...
	// V2 에서 생성자를 만들어서 객체들을 초기화 하는 코드를 사용한다.
	...
	public BuyerDto findByBuId() {
		...
		while (true) {
			...
			if (buyerDto == null) {
				HelpMessage.ERROR(String.format("입력한 ID의 고객정보가 없습니다. (%s)", strBuId));
				continue;
			}
			return buyerDto;
		}
	...
	protected String inputBuId() {
		while (true) {
			...
			System.out.println("고객ID 를 입력해 주세요. QUIT : 종료");
			System.out.print("고객ID >> ");
			...
			if (strBuId.isBlank()) {
				HelpMessage.COMFIRM("고객ID가 없습니다. ", "고객ID 를 새로 생성할까요?(Y:생성) ");
	...
	public void insert() {
		...
		while (true) {
			...
			if (buyerDto == null) {
				...
				String message = String.format("등록된 고객 ID 입니다. \n\t" + "고객명 : (%s)\n\t" + "전화번호 : (%s) ",
		...
		while (true) {
			...
			if (buyerDto.buAddr == null && strAddr.isEmpty()) {
				HelpMessage.ERROR("고객주소는 반드시 입력해야 합니다. ");
				continue;
		...
		String okMessage = String.format("고객ID : %s\n" + "고객이름 : %s\n" + "고객전화번호 : %s\n" + "고객주소 : %s\n", buyerDto.buId,
				buyerDto.buName, buyerDto.buTel, buyerDto.buAddr);
